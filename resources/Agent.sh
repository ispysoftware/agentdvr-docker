...
if [[ "$UID_CHANGED" == 'true' ]] && [[ "$GID_CHANGED" == 'true' ]]; then
        ...
        for ((i = 128; i <= 150; i++)); do
                if [ -e /dev/dri/renderD"$i" ]; then
                        GPU_PERMISSION=$(stat --format '%a' /dev/dri/renderD"$i")
                        if [ "$GPU_PERMISSION" -ne 666 ]; then
    ...
    su -m agentdvr -c "/AgentDVR/Agent $AGENT_CONFIG_PARAM"
else
    # run as root if user creation failed (default behavior).
    echo "PUID/PGID is NOT SET thus Running AgentDVR as $(whoami)..."
    su -m "$(whoami)" -c "/AgentDVR/Agent $AGENT_CONFIG_PARAM"
fi
}
CURRENT_PORT=$(cat /AgentDVR/Media/XML/current_port.txt)
CURRENT_PORT=$(expr "$CURRENT_PORT")
WEBUI_PORT=$(expr "$WEBUI_PORT")
